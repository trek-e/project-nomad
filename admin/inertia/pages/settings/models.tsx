        ...
        <main className="px-12 py-6">
          ...
          {isInstalled && systemInfo?.gpuHealth?.status === 'passthrough_failed' && !gpuBannerDismissed && (
            <Alert
              ...
              message={`Your system has ${systemInfo?.gpuHealth?.gpuType === 'amd' ? 'an AMD' : systemInfo?.gpuHealth?.gpuType === 'intel' ? 'an Intel Arc' : 'an NVIDIA'} GPU, but ${aiAssistantName} can't access it. AI is running on CPU only, which is significantly slower.`}
              className="!mt-6"
              dismissible={true}
